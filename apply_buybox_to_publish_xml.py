#!/usr/bin/env python3
"""
Buybox koruması için publish-stokmont-xml branch'indeki XML'e uygulama scripti
"""
...
# Logging setup
logging.basicConfig(
    filename='buybox_protection_publish.log',
    level=logging.INFO,
...
class BuyboxProtectionTransformer:
    def __init__(self):
        ...
        self.BRAND = "SDSTEP"
        self.SALT = "SDSTEP_BUYBOX_PROTECTION_2025"
    ...
    def transform_xml(self, input_file: str, output_file: str):
        ...
        logging.info("Buybox koruma dönüşümü başladı (publish-stokmont-xml)")
        ...
        for product in root.findall('.//Product'):
            ...
            # Ürün barkodunu dönüştür
            barcode_elem = product.find('Barcode')
            if barcode_elem is not None:
                original_barcodes.add(barcode_elem.text)
                base_string = f"{product_code}_{product_name}_{category}"
                new_barcode = self.generate_deterministic_barcode(base_string, self.PRODUCT_PREFIX)
        ...
        print("✅ Buybox koruma dönüşümü tamamlandı (publish-stokmont-xml)!")
        print(f"📦 İşlenen: {product_count} ürün, {variant_count} varyant")
        print(f"🔢 Üretilen barkod: {len(new_barcodes)}")
        print(f"📝 Log dosyası: buybox_protection_publish.log")
